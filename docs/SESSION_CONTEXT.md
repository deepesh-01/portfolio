...
>
> **Last updated:** 2026-05-12, after the **Strategic Command Center retool** *and* the **compensation scrub**. The home page is now a **persona-gated hybrid dashboard** (Founder / Engineering / TA-HR) with an **AI-Native Ecosystem capstone**, **Phase Bento** with expandable Memoir toggles, **Field Notes** visual milestones, and a **Reserved Warrior trajectory banner** (two cells: Status, Available For). ADR-0018 (Persona-Gated Hybrid Dashboard) added. Two new source docs: `docs/memoir_phase_9_ai_native_infrastructure.md` (AI-native primitives — Rspack, Docker-less swarm, 10× QA loop) and `docs/marketing_cover_page_strategy.md` (the persona positioning doc). Identity bumped to **Founding Member & Institutional Technical Guardian**. **Compensation scrub (2026-05-12, irreversible policy):** every LPA / fixed / variable / ESOP-count figure stripped from the public site (index.html, founder.html, identity.md, journey.md, journal.md, perspectives.md, marketing strategy, ADR-0018). Numbers replaced with framing ("the package was restructured around equity", "terms are a private conversation, not a public number"). ADR count: 17 → 18. Asset cache-bust: `styles.css?v=17`, `covers.css?v=2`, `app.js?v=18`. Also: ADR Prev/Next nav strip added — every `/#XXXX-...` view ends with `[Prev] [ADR N / 18] [Next]`, endpoints disabled at ADR-0001 / ADR-0018.
>
